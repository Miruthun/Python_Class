...
2. What does the list method `pop()` do?
   ...
   b) Removes and returns the last element : Answer
...
4. What does the `remove()` method do in a list?
   ...
   b) Removes the first occurrence of a value : Answer
   c) Removes all elements
...
8. What does the `union()` method do in a set?
   ...
   b) Returns all elements from both sets : Answer
...
9. What does the `intersection()` method return?
   ...
   b) Elements common to both sets : Answer
...
13. What does the `get()` method do in a dictionary?
    ...
    b) Returns the value for a key, or None if not found : Answer
...
15. What does the `update()` method do in a dictionary?
    a) Merges another dictionary or key-value pairs : Answer
...
22. What does the `difference()` method do in a set?
    ...
    b) Returns elements in the first set but not the second : Skip
...
24. What does the `copy()` method do in a list?
    a) Clears the list
    b) Returns a shallow copy of the list : Answer
    c) Reverses the list
    d) Sorts the list
...
27. What does the `insert()` method do in a list?
    ...
    b) Adds an element at a specific index : Answer
...
29. What does the `copy()` method do in a set?
    a) Clears the set
    b) Returns a shallow copy of the set : Answer
    c) Adds an element
...
31. What does the `symmetric_difference()` method return in a set?
    ...
    c) All elements in both sets : Skip
...
33. What does the `isdisjoint()` method check in a set?
    ...
    c) If two sets have no common elements : Skip
    d) If a set is empty
...
35. What does the `popitem()` method do in a dictionary?
    a) Removes and returns an arbitrary key-value pair : Answer
...
44. What does the `issuperset()` method check in a set?
    a) If a set contains another set : Skip
    b) If a set is empty
...
46. What does the `difference_update()` method do in a set?
    ...
    b) Removes elements present in another set : Skip
    c) Combines two sets
    d) Clears the set
...
48. What happens when you use `del dict[key]` on a dictionary?
    a) Removes the key-value pair : Skip
    b) Clears the dictionary
    c) Adds a key
...
50. What does the `intersection_update()` method do in a set?
    a) Keeps only elements common to both sets : Skip
